...
double sensor = 0, gap = 0, output = 0;
...
void loop() {
  ...
  if (s2 != s2a && s2) {
    myPID.Start(analogRead(A0), 0, spsuhu);
    tahap1 = true;
    selesai = false;
  }
  s2a = s2;
  sensor = analogRead(A7);
  gap = abs(myPID.GetSetpoint() - sensor);
  if (gap < 10) {
    myPID.SetTunings(consKp, consKi, consKd);
  } else {
    myPID.SetTunings(aggKp, aggKi, aggKd);
  }
  if (tahap1 || tahap2) output = myPID.Run(sensor);
  else output = 0;
  ...
  if (tahap2 && ada_error == 0) {
    digitalWrite(A0, LOW);
    digitalWrite(A1, HIGH);
    digitalWrite(A2, LOW);
    digitalWrite(A3, LOW);
  }
  if (tahap3 && ada_error == 0) {
    digitalWrite(A0, LOW);
    digitalWrite(A1, LOW);
    digitalWrite(A2, HIGH);
    digitalWrite(A3, LOW);
  }
  if (tahap4 && ada_error == 0) {
    digitalWrite(A0, LOW);
    digitalWrite(A1, LOW);
    digitalWrite(A2, LOW);
